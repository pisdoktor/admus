...
class IlHTML {
	static function editIl($row) {
		?>
		<div id="module_header">İl <?php echo $row->id ? 'Düzenle' : 'Ekle';?></div>
		<div id="module">
...
<input type="hidden" name="bolum" value="il" />
...
<form action="index.php" method="post" name="adminForm">
<div align="right">
<input type="button" name="button" value="Yeni İl Ekle" onclick="javascript:submitbutton('add');" class="button" />
...
<input type="button" name="button" value="Seçileni Sil" onclick="javascript:if (document.adminForm.boxchecked.value == 0){ alert('Lütfen listeden bir seçim yapın'); } else if (confirm('Bu illeri silmek istediğinize emin misiniz?')){ submitbutton('delete');}" class="button" /> 
</div>
...
<td width="90%">
<center>
<a href="index.php?option=yonetim&bolum=il&task=editx&id=<?php echo $row->id;?>">
<?php echo $row->name;?>
</a>
</center>
</td>
...
<?php 
$link = 'index.php?option=yonetim&bolum=il';
