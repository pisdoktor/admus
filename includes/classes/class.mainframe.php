  ...
  /**
	...
	*/
	function initSession() {
		...
		$session     = new admSession( $this->_db );
	...
	*/
	function login( $username=null, $passwd=null, $remember=0, $userid=NULL ) {
		...
		$row = null;
		if (!$username || !$passwd) {
			mosRedirect('index.php', 'Lütfen kullanıcı adı, parola ve sicil no alanlarını doldurunuz.');
			exit();
		} else {
			...
			// query used for remember me cookie
				...
				$query = "SELECT id, name, username, password, groupid"
				. "\n FROM #__users"
				. "\n WHERE id = " . (int) $userid
				;
			...
			// query used for login via login module
				$query = "SELECT u.id, u.name, u.username, u.password, u.groupid, g.folder"
				. "\n FROM #__users AS u"
				. "\n LEFT JOIN #__usergroups AS g ON g.id=u.groupid"
			...
			if (is_object($row)) {
				...
				$session->groupid    = intval( $row->groupid ); 
				   
				if ($row->groupid == 1) {
				$this->_isAdmin = true;
					$session->access_type = 'admin';
				} else if ($row->groupid == 2) {
					$session->access_type = 'doktor';
				} else if ($row->groupid == 3) {
					$session->access_type = 'kolluk';    
				} else if ($row->groupid == 4) {
					$session->access_type = 'savcilik';
	...
	*/
	...
	function getUser() {
		...
		$user = new admUsers( $this->_db );
		
		$user->id           = intval( $this->_session->userid );
		$user->username     = $this->_session->username;
		$user->groupid      = $this->_session->groupid;
		$user->sessionid    = $this->_session->session;
		$user->access_type  = $this->_session->access_type;
				
		if ($user->id) {
			$query = "SELECT u.id, u.name, u.email, u.lastvisit, u.kurumid, k.name as kurumadi, g.name AS usergroup, i.id AS ilid, ii.id AS ilceid"
			. "\n FROM #__users AS u"
			. "\n LEFT JOIN #__kurumlar AS k ON k.id=u.kurumid"
			. "\n LEFT JOIN #__usergroups AS g ON g.id=u.groupid"
			...
			$user->kurumid      = $my->kurumid;
			$user->kurumadi     = $my->kurumadi;
			$user->usergroup    = $my->usergroup;
			$user->ilid         = $my->ilid;
			$user->ilceid       = $my->ilceid;
		}
